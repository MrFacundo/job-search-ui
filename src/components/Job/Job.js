...
export default function Job({ job }) {
	...
	return (
		<Card className="mb-3 bg-light shadow-sm bg-white rounded">
			<Card.Body>
					...
					<div style={{ width: "80%" }}>
						...
						<div className="d-flex justify-content-between">
							{attributes.location.city && (
								<Card.Subtitle>{attributes.location.city}</Card.Subtitle>
							)}
							<Card.Subtitle className="text-muted mb-2">
								{Math.floor(
									(new Date(Date.now()) -
										new Date(attributes.publications[0].publicationTime)) /
										(1000 * 3600 * 24)
								)}{" "}
								days ago
							</Card.Subtitle>
						...
						{attributes.workingTimes[0] && (
							<Badge variant="secondary" className="mr-2">
								{attributes.workingTimes[0].title}
							</Badge>
						)}

						{attributes.employmentTypes[0] &&
							attributes.employmentTypes[0].title && (
